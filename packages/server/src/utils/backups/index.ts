...
export const keepLatestNBackups = async (
	...
	try {
		...
		const remoteBase = getDestinationRemoteBase(destination);
		const backupFilesPath = path.join(remoteBase, backup.prefix);
		...
		let rcloneCommand = `${rcloneList} | ${sortAndPickUnwantedBackups} ${rcloneDelete}`;
		rcloneCommand = wrapWithDestinationSetup(destination, rcloneCommand);
