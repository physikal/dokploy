...
export const restoreWebServerBackup = async (
	...
	try {
		...
		const backupPath = getDestinationRemotePath(destination, backupFile);
		...
		try {
			...
			let downloadCmd = `rclone copyto ${rcloneFlags.join(" ")} "${backupPath}" "${tempDir}/${backupFile}"`;
			downloadCmd = wrapWithDestinationSetup(destination, downloadCmd);
