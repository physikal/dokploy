	...
	listBackupFiles: protectedProcedure
		...
		.query(async ({ input }) => {
			try {
				...
				const remotePath = getDestinationRemoteBase(destination);
				...
				const searchPath = baseDir ? `${remotePath}/${baseDir}` : remotePath;
				let listCommand = `rclone lsjson ${rcloneFlags.join(" ")} "${searchPath}" --no-mimetype --no-modtime 2>/dev/null`;
				listCommand = wrapWithDestinationSetup(destination, listCommand);
